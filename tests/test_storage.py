...
class storageTest(unittest.TestCase):
    def setUp(self):
        ...
        self.tags = {'tag': 1, 'tag2': 2, 'tag3': 'lol'}
        self.notebooks = {'notebook': 'mylaptop'}
    ...
    def test_get_user_info_success(self):
        self.assertEqual(self.storage.getUserInfo(), self.userinfo)

    def test_get_user_props_success(self):
        props = [{'oAuthToken': 'testoauthtoken'},
                 {'info': {'email': 'test@mail.com'}}]
        self.assertEqual(self.storage.getUserprops(), props)

    def test_get_user_props_exists_success(self):
        self.assertEqual(self.storage.getUserprop('info'),
                          self.userinfo)
    ...
    def test_get_tags_success(self):
        tags = {'tag': '1', 'tag2': '2', 'tag3': 'lol'}
        self.assertTrue(self.storage.setTags(self.tags))
        self.assertEqual(self.storage.getTags(), tags)

    def test_replace_tags_success(self):
        tags = {'tag': '1', 'tag2': '2', 'tag3': '3'}
        self.assertTrue(self.storage.setTags(self.tags))
        self.tags['tag3'] = 3
        self.assertTrue(self.storage.setTags(self.tags))
        self.assertEqual(self.storage.getTags(), tags)
    ...
    def test_set_notebooks_success(self):
        self.assertEqual(self.storage.getNotebooks(), {})
        self.storage.setNotebooks(self.notebooks)
        self.assertEqual(self.storage.getNotebooks(), self.notebooks)

    def test_replace_notebooks_success(self):
        newnotebooks = {'notebook': 'android'}
        self.storage.setNotebooks(self.notebooks)
        self.storage.setNotebooks(newnotebooks)
        self.assertEqual(self.storage.getNotebooks(), newnotebooks)
